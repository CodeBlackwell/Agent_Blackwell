...
class Orchestrator:
    ...
    async def get_task_status(self, task_id: str) -> Dict[str, Any]:
        ...
        results = self.redis_client.xrevrange(
            self.result_stream,
            count=100  # Limit to recent results
        )
        ...
        tasks = self.redis_client.xrevrange(
            self.task_stream,
            count=100  # Limit to recent tasks
        )
        ...
        try:
            ...
            if task_type == "dummy":
                result = {
                    "task_id": task_id,
                    "status": "completed",
                    "result": "pong" if task.get("input") == "ping" else "echo: " + str(task.get("input", ""))
                }
            elif task_type == "echo":
...
async def main():
    ...
    try:
        ...
        if "spec_agent" in orchestrator.agents:
            ...
            user_request = "Create a FastAPI service with PostgreSQL database for a todo app"
            task_id = await orchestrator.enqueue_task("spec_agent", {"input": user_request})
        ...
        for stream_name, messages in tasks:
            for message_id, message in messages:
                # Parse task
                task_data = json.loads(message[b"task"].decode("utf-8"))
                print(f"Processing task: {task_data['task_id']} of type {task_data['task_type']}")
