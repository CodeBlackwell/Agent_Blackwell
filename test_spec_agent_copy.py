...
class TestSpecAgentIntegration(BaseAgentIntegrationTest):
    ...
    async def test_spec_agent_with_mock_llm(
        self, mock_post, redis_client, agent_fixtures, user_request_fixtures
    ):
    ...
    async def test_spec_agent_handles_complex_request(
        ...
        complex_spec_output = agent_fixtures["spec"]["sample_outputs"][
            1
        ...
        assert (
            "technical_requirements" in output_data
        ), "technical_requirements missing for complex request"
    ...
    async def test_spec_agent_handles_llm_error(
        self, mock_post, redis_client, user_request_fixtures
    ):
        ...
        message_id, message_data = await self.wait_for_agent_output(
            redis_client, "spec", timeout=10.0
        )
    ...
    async def test_spec_agent_persistence(
        self, redis_client, agent_fixtures, user_request_fixtures
    ):
        ...
        assert (
            stored_message["spec_details"] == output_message["spec_details"]
