...
class TestCodingAgentIntegration(BaseAgentIntegrationTest):
    ...
    async def test_coding_agent_with_mock_llm(self, mock_post, redis_client, agent_fixtures, user_request_fixtures):
        ...
        expected_output_keys = ["request_id", "source_code", "file_structure", "dependencies", "documentation"]
        ...
        if file_structure:
            assert isinstance(file_structure[0], dict), "file structure item should be a dictionary"
        ...
        assert "setup_instructions" in documentation, "documentation missing setup_instructions"
    
    @pytest.mark.asyncio
    @patch("aiohttp.ClientSession.post")
    async def test_coding_agent_handles_multiple_services(self, mock_post, redis_client, agent_fixtures):
        """Test CodingAgent with multiple microservices."""
        # Setup mock LLM response for multi-service code generation
        complex_coding_output = agent_fixtures["coding"]["sample_outputs"][1]
        mock_response = await self.setup_mock_llm_response(complex_coding_output)
        mock_post.return_value.__aenter__.return_value = mock_response
        ...
        input_message = {
            ...
            "user_id": "test-user-456",
            "architecture": {
                "components": ["api-gateway", "user-service", "order-service", "payment-service", "notification-service"],
        ...
        expected_output_keys = [
            "request_id", "source_code", "file_structure", "dependencies", 
            "documentation", "deployment_config", "docker_config"
        ]
        ...
        assert "deployment_config" in output_data, "deployment_config missing for multi-service"
    ...
    @pytest.mark.asyncio
    @patch("aiohttp.ClientSession.post")
    async def test_coding_agent_handles_llm_error(self, mock_post, redis_client, agent_fixtures):
        """Test CodingAgent handles LLM errors gracefully."""
        # Setup mock LLM error response
        mock_response = AsyncMock()
        mock_response.status = 429  # Rate limit error
        mock_response.text = AsyncMock(return_value=json.dumps({"error": "Rate limit exceeded"}))
        mock_post.return_value.__aenter__.return_value = mock_response
        ...
        input_message = {
            "request_id": "error-coding-123",
            "user_id": "test-user-456",
            "architecture": design_output,
            "timestamp": "2025-06-26T01:35:00Z",
            "request_type": "coding",
            "priority": "normal"
        }
        ...
        message_id, message_data = await self.wait_for_agent_output(redis_client, "coding", timeout=15.0)
    ...
    @pytest.mark.asyncio
    async def test_coding_agent_persistence(self, redis_client, agent_fixtures):
        ...
        output_message = {
            ...
            "dependencies": {
                "requirements": ["fastapi>=0.104.0", "sqlalchemy>=2.0.0", "redis>=4.5.0"],
        ...
        assert "file_structure" in stored_message, "file_structure not persisted correctly"
    ...
    @pytest.mark.asyncio
    async def test_coding_agent_design_to_code_transition(self, redis_client, agent_fixtures):
        ...
        design_message_id = await redis_client.xadd(design_output_stream, design_output_message)
        ...
        coding_message_id = await redis_client.xadd(coding_input_stream, coding_input_message)
        
        # Verify both messages exist
        design_messages = await redis_client.xrange(design_output_stream, min=design_message_id, max=design_message_id)
        coding_messages = await redis_client.xrange(coding_input_stream, min=coding_message_id, max=coding_message_id)
        ...
        assert design_stored["request_id"] == coding_stored["request_id"], "request_id not consistent across transition"
        assert coding_stored["previous_agent"] == "design", "previous_agent not set correctly"
        assert "architecture" in coding_stored, "architecture not passed to coding agent"
    ...
    @pytest.mark.asyncio
    @patch("aiohttp.ClientSession.post")
    async def test_coding_agent_code_quality_features(self, mock_post, redis_client, agent_fixtures):
        """Test CodingAgent includes code quality features."""
        # Setup mock LLM response with quality-focused output
        quality_coding_output = agent_fixtures["coding"]["sample_outputs"][0]
        ...
        expected_output_keys = [
            "request_id", "source_code", "file_structure", "dependencies", 
            "documentation", "test_files", "linting_config"
        ]
        ...
        assert "pyproject.toml" in linting_config or ".flake8" in linting_config, "linting config files missing"
