...
class TestReviewAgentIntegration(BaseAgentIntegrationTest):
    ...
    async def test_review_agent_with_mock_llm(
        self,
        mock_post,
        redis_client,
        agent_worker,
        ...
        input_message = {
            ...
            "dependencies": {"requirements": ["fastapi>=0.104.0", "sqlalchemy>=2.0.0"]},
        ...
        assert (
            "code_quality_score" in output_data
        ), "code_quality_score missing from output"
        quality_score = output_data["code_quality_score"]
        assert isinstance(
            quality_score, (int, float)
        ), "code_quality_score should be numeric"
        assert (
            0 <= quality_score <= 100
        ), "code_quality_score should be between 0 and 100"
    ...
    @pytest.mark.asyncio
    @patch("aiohttp.ClientSession.post")
    async def test_review_agent_handles_security_issues(
        self, mock_post, redis_client, agent_worker, agent_fixtures
    ):
        ...
        # Test agent output
        output_data = await self.assert_agent_produces_output(
            redis_client, "review", input_message, expected_output_keys, timeout=15.0
        )
        ...
        assert output_data["approval_status"] in [
            "rejected",
            "needs_revision",
        ], "Vulnerable code should not be approved"

    @pytest.mark.asyncio
    @patch("aiohttp.ClientSession.post")
    async def test_review_agent_handles_performance_analysis(
        self, mock_post, redis_client, agent_worker, agent_fixtures
    ):
        """Test ReviewAgent analyzes performance concerns."""
        # Setup mock LLM response focused on performance review
        performance_review_output = agent_fixtures["review_agent"]["outputs"][0][
            "review"
        ]
        ...
        assert (
            "performance_analysis" in output_data
        ), "performance_analysis missing for performance review"
        performance = output_data["performance_analysis"]
        assert isinstance(
            performance, dict
        ), "performance_analysis should be a dictionary"
        assert (
            "bottlenecks" in performance
        ), "bottlenecks missing from performance analysis"
        assert (
            "optimization_suggestions" in performance
        ), "optimization_suggestions missing"

        # Should find performance issues
        issues = output_data["issues_found"]
        performance_issues = [
            issue for issue in issues if issue.get("category") == "performance"
        ]
        assert (
            len(performance_issues) > 0
        ), "No performance issues found in inefficient code"

    @pytest.mark.asyncio
    @patch("aiohttp.ClientSession.post")
    async def test_review_agent_handles_llm_error(
        self, mock_post, redis_client, agent_worker, agent_fixtures
    ):
        ...
        mock_response.text = AsyncMock(
            return_value=json.dumps({"error": "Internal Server Error"})
        )
        mock_post.return_value.__aenter__.return_value = mock_response
        ...
        message_id, message_data = await self.wait_for_agent_output(
            redis_client, "review", timeout=15.0
        )
    ...
    @pytest.mark.asyncio
    async def test_review_agent_persistence(
        self, redis_client, agent_worker, agent_fixtures
    ):
        ...
        assert stored_message["code_quality_score"] == str(
            output_message["code_quality_score"]
        ), "code_quality_score not persisted correctly"
        assert (
            stored_message["approval_status"] == output_message["approval_status"]
        ), "approval_status not persisted correctly"

        # Cleanup
        await redis_client.xdel(output_stream, message_id)

    @pytest.mark.asyncio
    async def test_review_agent_coding_to_review_transition(
        self, redis_client, agent_worker, agent_fixtures
    ):
        ...
        if isinstance(code_data.get("source_code"), dict):
            ...
            for file_info in code_data["files"]:
                if (
                    ...
                    and "path" in file_info
                    and "content" in file_info
                ):
        ...
        # Test the transition
        output_data = await self.assert_agent_produces_output(
            redis_client, "review", review_input, expected_keys, timeout=30.0
        )

        # Verify the transition worked correctly
        assert (
            output_data["request_id"] == review_input["request_id"]
        ), "request_id should match"

        # Verify review-specific fields
        assert "code_quality_score" in output_data, "code_quality_score missing"
        quality_score = output_data["code_quality_score"]
        assert isinstance(quality_score, (int, float)) or (
            isinstance(quality_score, str) and quality_score.replace(".", "").isdigit()
        ...
        assert approval_status in valid_statuses or (
            isinstance(approval_status, str)
            and approval_status.strip('"') in valid_statuses
        )
