...
class TestSpecAgentIntegration(BaseAgentIntegrationTest):
    ...
    async def test_spec_agent_with_mock_llm(self, mock_post, redis_client, agent_fixtures, user_request_fixtures, agent_worker):
        ...
        print(f"DEBUG: Output keys: {list(output_data.keys()) if output_data else 'None'}")
    ...
    @pytest.mark.asyncio
    @patch("aiohttp.ClientSession.post")
    async def test_spec_agent_handles_complex_request(self, mock_post, redis_client, agent_fixtures, user_request_fixtures, agent_worker):
        """Test SpecAgent with complex request."""
        # Setup mock LLM response for complex request
        complex_spec_output = agent_fixtures["spec_agent"]["outputs"][1]  # Using second sample output
    ...
    @pytest.mark.asyncio
    @patch("aiohttp.ClientSession.post")
    async def test_spec_agent_handles_llm_error(self, mock_post, redis_client, user_request_fixtures, agent_worker):
        ...
        message_id, message_data = await self.wait_for_agent_output(redis_client, "spec_agent", timeout=10.0)
    ...
    @pytest.mark.asyncio
    async def test_spec_agent_persistence(self, redis_client, agent_fixtures, user_request_fixtures, agent_worker):
