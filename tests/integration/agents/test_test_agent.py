...
class TestTestAgentIntegration(BaseAgentIntegrationTest):
    ...
    async def test_test_agent_with_mock_llm(self, mock_post, redis_client, agent_fixtures, user_request_fixtures):
        ...
        expected_output_keys = ["request_id", "test_files", "test_coverage", "test_results", "performance_tests"]
        ...
        assert "performance_tests" in output_data, "performance_tests missing from output"
    ...
    async def test_test_agent_generates_comprehensive_tests(self, mock_post, redis_client, agent_fixtures):
        ...
        expected_output_keys = [
            "request_id", "test_files", "test_coverage", "test_results", 
            "performance_tests", "integration_tests", "security_tests"
        ]
        ...
        assert "integration_tests" in output_data, "integration_tests missing for comprehensive testing"
        integration_tests = output_data["integration_tests"]
        assert isinstance(integration_tests, dict), "integration_tests should be a dictionary"
        assert len(integration_tests) > 0, "integration_tests is empty"
        
        # Verify security tests
        assert "security_tests" in output_data, "security_tests missing for comprehensive testing"
        ...
        assert coverage_percentage >= 80, f"Coverage {coverage_percentage}% too low for comprehensive testing"
    ...
    async def test_test_agent_handles_test_failures(self, mock_post, redis_client, agent_fixtures):
        ...
        failed_test_output = json.dumps({
            ...
            "test_coverage": {"percentage": 60, "covered_lines": 30, "total_lines": 50},
            "test_results": {
                "passed": 5,
                "failed": 3,
                "total": 8,
    ...
    async def test_test_agent_performance_testing(self, mock_post, redis_client, agent_fixtures):
        ...
        assert "avg_response_time" in load_test, "avg_response_time missing from load test"
        ...
        assert "breaking_point" in stress_test, "breaking_point missing from stress test"
        assert "memory_usage_mb" in stress_test, "memory_usage_mb missing from stress test"
    ...
    async def test_test_agent_handles_llm_error(self, mock_post, redis_client, agent_fixtures):
        ...
        mock_response.text = AsyncMock(return_value=json.dumps({"error": "Gateway Timeout"}))
        mock_post.return_value.__aenter__.return_value = mock_response
    ...
    async def test_test_agent_persistence(self, redis_client, agent_fixtures):
        ...
        assert "test_coverage" in stored_message, "test_coverage not persisted correctly"
    ...
    async def test_test_agent_review_to_test_transition(self, redis_client, agent_fixtures):
        ...
        review_output_message = {
            ...
            "approval_status": "approved",
            "issues_found": [],
            "recommendations": ["Add edge case tests", "Include performance benchmarks"],
        ...
        test_messages = await redis_client.xrange(test_input_stream, min=test_message_id, max=test_message_id)
        ...
        assert review_stored["request_id"] == test_stored["request_id"], "request_id not consistent across transition"
        assert test_stored["previous_agent"] == "review", "previous_agent not set correctly"
        assert "source_code" in test_stored, "source_code not passed to test agent"
        assert test_stored["approval_status"] == "approved", "approval_status not passed correctly"
