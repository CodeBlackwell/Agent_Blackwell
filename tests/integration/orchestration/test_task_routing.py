...
class TestTaskRouting:
    ...
    async def test_task_enqueue_and_route(self, orchestrator_client):
        ...
        for task_type in task_types:
            ...
            # Parse and verify task content
            task_json = json.loads(task_payload[b"task"].decode())
            assert task_json["task_type"] == task_type
            assert task_json["task_data"]["description"] == f"Test {task_type} task"
    ...
    async def test_task_lifecycle_management(self, orchestrator_client):
        ...
        assert call_args["input"] == "Create API specification"
    ...
    async def test_agent_coordination_workflow(self, orchestrator_client):
        ...
        tasks = [
            ...
            },
            {
                ...
                "task_type": "design",
                "task_data": {"description": "Design system", "spec_result": spec_result}
            },
            {
                "task_id": "workflow-coding-1",
                "task_type": "coding", 
                "task_data": {"description": "Implement code", "design_result": design_result}
            }
        ]
...
class TestPerformanceAndConcurrency:
    ...
    async def test_task_queue_management(self, orchestrator_client):
        ...
        task_id = await orchestrator_client.enqueue_task("spec", {
            "description": "Test queue management",
            "priority": "normal"
        })
