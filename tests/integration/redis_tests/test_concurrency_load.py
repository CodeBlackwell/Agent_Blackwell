...
class TestRedisConcurrency:
    ...
    async def test_multiple_concurrent_producers(self, redis_client, sample_tasks):
        ...
        async def producer_task(producer_id: int, tasks: List[Dict]):
            ...
            producer_client = redis.Redis(
                host="redis-test", port=6379, decode_responses=True
            )
        ...
        producer_tasks = [producer_task(i, sample_tasks) for i in range(num_producers)]
        ...
        print(
            f"Concurrent producers test completed in {end_time - start_time:.2f} seconds"
        )
    ...
    async def test_multiple_concurrent_consumers(self, redis_client, sample_tasks):
        ...
        print(
            f"Concurrent consumers test completed in {end_time - start_time:.2f} seconds"
        )
...
class TestRedisLoadTesting:
    ...
    async def test_sustained_load_processing(self, redis_client):
        ...
        print(
            f"Processing rate: {total_processed / actual_duration:.1f} messages/second"
        )
        ...
        processing_efficiency = (
            total_processed / produced_count if produced_count > 0 else 0
        )
