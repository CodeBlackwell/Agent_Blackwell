    ...
    async def test_connection_drop_during_publishing(self, redis_client):
        ...
        with patch.object(redis_client, 'xadd') as mock_xadd:
            ...
            with pytest.raises(ConnectionError):
                await redis_client.xadd(stream_name, {"task_id": "fail_1", "content": "Failed message"})
            
            # Second attempt also fails
            with pytest.raises(ConnectionError):
                await redis_client.xadd(stream_name, {"task_id": "fail_2", "content": "Failed message"})
            
            # Third attempt succeeds (connection recovered)
            result = await redis_client.xadd(stream_name, {"task_id": "recovery", "content": "Recovery message"})
    ...
    async def test_connection_drop_during_consumption(self, redis_client):
        ...
        with patch.object(redis_client, 'xreadgroup') as mock_xreadgroup:
            ...
            mock_xreadgroup.side_effect = [
                [(stream_name, [(message_ids[0], {"task_id": "consume_test_0", "content": "Message 0"})])],
                ConnectionError("Connection lost during consumption"),
                [(stream_name, [(message_ids[1], {"task_id": "consume_test_1", "content": "Message 1"})])]
            ]
...
class TestRedisDuplicateHandling:
    ...
    async def test_idempotent_processing(self, redis_client):
        ...
        for attempt in range(3):
            msg_id = await redis_client.xadd(stream_name, {
                **message,
            ...
            if existing_result is None:
                ...
                await redis_client.hset(result_hash, message["task_id"], json.dumps(result))
