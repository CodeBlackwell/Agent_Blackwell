...
from src.orchestrator.agent_nodes import testing_agent_node  # Test agent node function
...
class TestDesignAgentNode:
    ...
    async def test_design_agent_node_no_spec_tasks(self, mock_design_agent):
        ...
        assert result_state["agent_results"][0]["error_message"] is not None
        assert (
            "No spec tasks available"
            in result_state["agent_results"][0]["error_message"]
        )
        assert result_state["agent_results"][0]["result"] is None
...
class TestStatePersistence:
    ...
    async def test_state_persistence_across_nodes(
        self, mock_design_agent, mock_spec_agent
    ):
