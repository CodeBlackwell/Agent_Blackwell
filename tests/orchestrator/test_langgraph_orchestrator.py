...
from src.orchestrator.workflow_state import WorkflowStatus, AgentWorkflowState, create_initial_state
...
class TestWorkflowExecution:
    ...
    async def test_execute_workflow_success(self, mock_create_state, orchestrator):
        ...
        mock_final_state = {
            **mock_initial_state,
            ...
            "agent_results": [{"agent_type": "spec", "success": True, "result": {"tasks": ["Task 1", "Task 2"]}}],
        ...
        with patch.object(orchestrator.workflow_graph, 'ainvoke', return_value=mock_final_state):
    ...
    async def test_execute_workflow_with_custom_task_type(self, orchestrator):
        ...
        with patch.object(orchestrator.workflow_graph, 'ainvoke', return_value=mock_state):
...
class TestStreamWorkflowExecution:
    ...
    async def test_stream_workflow_execution(self, orchestrator):
        ...
        result = [item async for item in orchestrator.stream_workflow_execution("test", "request")]
...
class TestErrorHandling:
    ...
    async def test_workflow_execution_error(self, mock_create_state, orchestrator):
        ...
        with patch.object(orchestrator.workflow_graph, 'ainvoke', return_value=error_state):
...
class TestWorkflowLifecycle:
    ...
    async def test_submit_and_execute_workflow(self, mock_uuid, orchestrator):
        ...
        with patch.object(orchestrator, 'submit_feature_request', return_value=mock_workflow_id) as mock_submit:
            ...
            mock_final_state = {
                ...
                "agent_results": [{"agent_type": "spec", "success": True, "result": {"tasks": ["Test task"]}, "execution_time": 1.5}],
                "completed_agents": ["spec"],
                "failed_agents": [],
                "error_messages": [],
                "created_at": "2025-06-25T18:56:00-04:00",
            ...
            with patch.object(orchestrator, 'execute_workflow', return_value=expected_response) as mock_execute:
                # Execute workflow - will return our mocked expected response
                result = await orchestrator.execute_workflow(workflow_id, "Test feature")
                assert result["status"] == WorkflowStatus.COMPLETED
                assert result["workflow_id"] == workflow_id
